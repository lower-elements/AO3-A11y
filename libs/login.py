...
def login():
	...
	try:
		...
		if user != "": 
			...
			dlg.SetValue("") #quite obviously sets the value of the username field to be nothing. 
			...
			with open('configfiles/user.usr','w+') as f:
				f.write(login_dict['user']) #writes the username to user.usr so the program can access it next time ti opens. 
			g.session = g.AO3.Session(login_dict['user'],login_dict['password']) #again, creates the AO3 session. 
			g.wx.MessageBox('You have just logged in as: '+login_dict['user'],'logged in', g.wx.OK) #a dialog box that is comformation. 
			g.settings['account']=True #sets the account variable in settings to true. We can use this to prevent errors from crashing the program if not logged in users attempt to do something they are unable. 
		
	except:
		#if an error occurs in logging in, show a warning, save the settings then close the program. 
